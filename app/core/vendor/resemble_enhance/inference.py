...
def inference(model, dwav, sr, device, chunk_seconds: float = 30.0, overlap_seconds: float = 1.0):
    ...
    # [VENDOR-FIX] resample() 拿 waveform.dtype 当 torch.dtype 用。
    # 旧版允许 numpy 数组进来（然后 dtype 会是 numpy.dtypes.Float32DType），
    # 但 torch>=2.10 + numpy>=2 在 torch.arange(..., dtype=numpy_dtype) 上抛 TypeError。
    # 提前把 dwav 转成 torch.Tensor 一举两得：
    #   (1) 后续 dwav.dim() / .abs() / .to(device) / / 都是 tensor 方法；
    #   (2) waveform.dtype 传下去就是 torch.dtype，不再炸。
    if not torch.is_tensor(dwav):
        dwav = torch.as_tensor(dwav, dtype=torch.float32)
